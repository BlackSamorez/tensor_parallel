...
from tensor_parallel import Config, Sharded, TensorParallel, TensorParallelPreTrainedModel


@pytest.mark.parametrize("devices", [("cpu",) * 2, ("cpu",) * 3])
@pytest.mark.parametrize("model_name", ["bert-base-uncased"])
def test_zero_3_only(devices, model_name):
...
@pytest.mark.parametrize("model_name", ["bert-base-uncased"])
def test_no_zero_3(devices, model_name):
    model = BertModel.from_pretrained(model_name).to(devices[0])
    model_state_dict = model.state_dict()
