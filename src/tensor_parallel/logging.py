"""
Fancy colored loggging, borrowed from https://github.com/learning-at-home/hivemind/
All credit goes to Alexander Borzunov https://github.com/borzunov
"""
import logging
import os
import sys
...
def _initialize_if_necessary():
    ...
    with _init_lock:
        ...
        _enable_default_handler("tensor_parallel")
...
def get_logger(name: Optional[str] = None) -> logging.Logger:
    ...
    :note: By default, the log handler (that reads the ``LOGLEVEL`` env variable and uses
           the colored log formatter) is only applied to messages logged inside the package.
...
def use_log_handler(where: Union[HandlerMode, str]) -> None:
    """
    Choose loggers where the default log handler is applied. Options for the ``where`` argument are:

    * "in_tensor_parallel" (default): Use the log handler in the loggers of the ``tensor_parallel`` package.
                               Don't propagate their messages to the root logger.
    * "nowhere": Don't use the log handler anywhere.
                 Propagate the ``tensor_parallel`` messages to the root logger.
    * "in_root_logger": Use the tensor_parallel log handler in the root logger
    ...
    if _current_mode == HandlerMode.IN_TENSOR_PARALLEL:
        _disable_default_handler("tensor_parallel")
